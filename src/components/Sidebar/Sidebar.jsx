...
const PAK_AFG_INTEL = {
  what: 'Pakistan declared "open war" on Afghanistan on February 27, 2026, launching Operation Ghazab Lil Haq ("Righteous Fury") with massive airstrikes on Kabul, Kandahar, and Paktia. The operation followed months of escalating TTP terrorism inside Pakistan and cross-border clashes along the Durand Line. On February 26, Afghan Taliban forces attacked 53 locations along the 2,600 km border, capturing multiple Pakistani outposts. Pakistan claims 684 Taliban fighters killed; Taliban claims 100+ Pakistani soldiers killed. UN has verified 56 Afghan civilian deaths and 129 injuries (185 total civilian casualties) since Feb 26, including 24 children and 6 women. On March 16, a Pakistani airstrike allegedly hit Kabul\'s Omar Addiction Treatment Hospital \u2014 the UN says 143 killed, Taliban claims 400, Pakistan denies targeting the hospital. Afghan Taliban retaliated by striking deep into Pakistan \u2014 hitting Nur Khan Airbase in Rawalpindi, the 12th Division HQ in Quetta, and camps in Mohmand Agency. Over 115,000 Afghans have been displaced (UNHCR), with 3,000+ displaced in Pakistan.',
...
const SUDAN_TIMELINE_BASE = [
  ...
  { time: '2026-03-09T12:00:00Z', text: 'UN reports 14\u201315 million displaced \u2014 Sudan surpasses Syria as world\'s worst displacement crisis' },
  ...
  { time: '2026-03-07T14:00:00Z', text: 'UNHCR reports 14\u201315 million displaced (9.3M internal, 4.3M cross-border) \u2014 launches $1.6 billion appeal for 5.9 million across 7 neighboring countries' },
...
const SUDAN_INTEL = {
  what: 'Sudan\'s civil war between the Sudanese Armed Forces (SAF, Gen. al-Burhan) and the Rapid Support Forces (RSF, Gen. Hemedti) erupted on April 15, 2023 and is approaching 1,000 days. An estimated 150,000+ people have been killed (combining violence, famine and disease) and 14\u201315 million displaced (9.3M internal, 4.3M cross-border) \u2014 the world\'s largest displacement crisis. A London School of Hygiene study found 61,000 died in Khartoum State alone. Some 33.7 million need humanitarian assistance \u2014 two-thirds of Sudan\'s population \u2014 with 25.6 million facing acute hunger. The RSF seized most of Khartoum in mid-2023 and captured nearly all of Darfur by late 2023, with systematic ethnic cleansing of the Masalit people in West Darfur. The SAF launched a counteroffensive in late 2024, retaking key districts in Khartoum. Drone strikes from both sides have intensified. The conflict is now spilling across borders \u2014 Chad ordered total border closure in March 2026 after an RSF drone strike killed 17 Chadian civilians.',
...
export default function Sidebar({ onCountryClick, onOpenStocksModal, stocksData, stocksLastUpdated, stocksUpdating, militaryMode }) {
  ...
  const renderPakAfgCard = () => {
    ...
    const afgCivKilled = floorStat(findStat(pakStats, ['afghan', 'civilian'], ['killed', 'deaths', 'dead', 'casualties', 'toll']), '185+', pakFloors.afghan_civilian_killed);
    ...
    const talibanKilled = floorStat(findStat(pakStats, ['taliban'], ['killed', 'deaths', 'dead', 'casualties']), '684+', pakFloors.taliban_killed);
  ...
  const renderSudanCard = () => {
    ...
    const killed = floorStat(findStat(sudanStats, ['total', 'estimated', 'civilian'], ['killed', 'deaths', 'dead', 'toll']), '150,000+', sudanFloors.total_killed);
    const displaced = floorStat(findStat(sudanStats, ['displac', 'total', 'internal'], ['displac', 'people', 'million']), '14-15M', sudanFloors.displaced);
